    ...
    with db.connect() as conn:
        ...
        search_results = conn.execute(
            sqlalchemy.text(
                ...
                "FROM amenities "
                "ORDER BY embedding_005 "
                "<-> embedding(:embedding_model, :search_term)::vector "
                "LIMIT 3 "
                ),
            parameters={"embedding_model": os.environ["EMBEDDING_MODEL"], "search_term": name}).fetchall()
        # Convert the results into a list of dicts representing votes
        for row in search_results:
            results.append({"name": row[0], "description": row[1], "location": row[2]})
